...
export default function ChatInterface({ initialSessionId = null, taskData = null, initialPrompt = null }) {
    ...
    const handleVoiceInput = useCallback(async (transcript, isFinal) => {
        ...
        setCurrentCharIndex(-1); // Reset char index

        // Pause listening immediately via callback if needed, but hook handles it via stopListening()
        // inside the silence timer.

        try {
            console.log('🌐 Sending to API:', transcript);
            // Use ref to get current session ID without re-creating callback
            const currentSessionId = sessionIdRef.current;
            const sidParam = currentSessionId ? `&session_id=${currentSessionId}` : '';
            const response = await api.post(`/api/student/chat/voice?text=${encodeURIComponent(transcript)}${sidParam}`);
            ...
            setMessages((prev) => [
                ...prev,
                { role: 'assistant', content: aiResponse, isVoice: true },
            ]);

            if (response.data.should_speak) {
                // Schedule speech
                setResponseToSpeak(aiResponse);
            }
        } catch (error) {
            console.error('❌ Voice chat error:', error);
            const errorMsg = error.response?.data?.detail || "Sorry, I couldn't process that. Please try again!";
            setMessages((prev) => [
                ...prev,
    ...
    }, []); // Empty deps as we use functional state updates
